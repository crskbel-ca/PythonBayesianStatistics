...
class _DictWrapper(object):
    ...
    def InitMapping(self, values):
        ...
        """
        for value, prob in values.items(): #@fix
            self.Set(value, prob)
    ...
    def Print(self):
        ...
        for val, prob in sorted(self.d.items()): #@fix
            print(val, prob)
    ...
    def Total(self):
        ...
        #@fix total = sum(self.d.itervalues())
        total = sum(iter(self.d))

        return total

    def MaxLike(self):
        """Returns the largest frequency/probability in the map."""
        #@fix return max(self.d.itervalues())
        return max(iter(self.d))
...
class Pmf(_DictWrapper):
    ...
    def ProbGreater(self, x):
        ...
        returns: float probability
        """
        t = [prob for (val, prob) in self.d.items() if val > x] #@fix
        return sum(t)
...
def MakeCdfFromDict(d, name=''):
    ...
    Returns:
        Cdf object
    """
    return MakeCdfFromItems(d.items(), name) #@fix
...
def SampleSum(dists, n):
    ...
    """
    pmf = MakePmfFromList(RandomSum(dists) for i in range(n))  #@fix
    return pmf
...
def MakePoissonPmf(lam, high, step=1):
    ...
    """
    pmf = Pmf()
    for k in range(0, high + 1, step):   #@fix
...
class Beta(object):
    ...
    def MakePmf(self, steps=101, name=''):
        ...
        xs = [i / (steps - 1.0) for i in range(steps)] #@fix
