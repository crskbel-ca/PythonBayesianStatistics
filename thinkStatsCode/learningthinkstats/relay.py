...
results = 'file:///C|/D/anaschool/programming/bitbucket/k12/python/thinkStatsCode/learningthinkstats/dataforrelay.txt'
...
def CleanLine(inputLine):
    ...
    t = inputLine.decode("utf-8").split()
    print("t:", t)
    if len(t) < 6:
        return None
...
def ReadResults(url=results):
    ...
    results = []
    conn = urllib.request.urlopen(url) #@todo: fix
    for line in conn.fp:
        t = CleanLine(line)
        if t:
            results.append(t)
...
1) results = ReadResults()
2) CleanLine()
3) speeds = GetSpeeds()
4) ConvertPaceToSpeed()
5) MakePmfFromList()
