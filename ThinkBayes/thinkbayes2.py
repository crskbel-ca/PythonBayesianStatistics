...
class _DictWrapper(object):
    ...
    def exp(self, m=None):
        ...
        """
        # NOTE: we are shifting up the distribution by m (max likliehood) to avoid underflow when
        # note: converting back to non-log form
        if not self.log:
...
class Cdf:
    ...
    def __init__(self, obj=None, ps=None, label=None):
        ...
        self.ps /= self.ps[-1] # normalizing: dividing by sum of all = last element in cdf
...
class Suite(PMF):
    ...
    def logUpdate(self, data):
        ...
        Args:
            data: any representation of the data
        """
        # NOTE: does not normalize, like update, because it would be nonsensical
        # help: why?
        for hypo in self.values():
