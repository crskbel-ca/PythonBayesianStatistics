...
def cdf(theCdf, complement=False, transform=None, **options):
    ...
    if theCdf.name:
        options = underride(options, label=theCdf.name)

    plot(xs, ps, **options)
    return scale


def cdfs(theCdfs, complement=False, transform=None, **options):
